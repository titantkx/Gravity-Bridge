...
- [run-test-all-no-reset.sh](run-test-all-no-reset.sh): automatically run all repeatable test cases.
...
2. Run test cases by running `./run-test.sh <test-case-name>`. Or run all repeatable test cases by running `./run-test-all-no-reset.sh`.
