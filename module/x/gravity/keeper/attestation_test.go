...
func TestGetAndDeleteAttestation(t *testing.T) {
	...
	for _, evmChain := range k.GetEvmChains(ctx) {
		_, _, hashes := createAttestations(t, length, k, ctx, evmChain.EvmChainPrefix)
...
func TestGetMostRecentAttestations(t *testing.T) {
	...
	length := 10
	for _, evmChain := range k.GetEvmChains(ctx) {
		msgs, anys, _ := createAttestations(t, length, k, ctx, evmChain.EvmChainPrefix)
...
func createAttestations(t *testing.T, length int, k Keeper, ctx sdktypes.Context, evmChainPrefix string) ([]types.MsgSendToCosmosClaim, []codectypes.Any, [][]byte) {
...
func TestGetSetLastObservedEthereumBlockHeight(t *testing.T) {
	...
	for _, evmChain := range k.GetEvmChains(ctx) {
		require.NotPanics(t, func() { k.SetLastObservedEthereumBlockHeight(ctx, evmChain.EvmChainPrefix, ethereumHeight) })

		ethHeight := k.GetLastObservedEthereumBlockHeight(ctx, evmChain.EvmChainPrefix)
...
func TestInvalidHeight(t *testing.T) {
	...
	lastEthHeight := pk.GetLastObservedEthereumBlockHeight(ctx, evmChain.EvmChainPrefix).EthereumBlockHeight
	lastBatchNonce := 0
